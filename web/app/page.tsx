...
export default function Home() {
  ...
  return (
    <div className="space-y-14 sm:space-y-20">
      <section className="rise space-y-7 pt-6 sm:space-y-9 sm:pt-20">
        <p className="badge">Bilirkişi aylarca inceleyecek · biz 3 saniyede</p>
        <h1 className="font-display text-[2.1rem] leading-[1.1] sm:text-[4.4rem] sm:leading-[1.08]">
        ...
        <p className="max-w-2xl font-body text-lg leading-relaxed text-fg-2 sm:text-xl">
        ...
        <ul className="flex flex-wrap gap-2" aria-label="Örnek aramalar">
          {EXAMPLES.map((q) => (
            <li key={q}>
              <Link href={`/ara?q=${encodeURIComponent(q)}`} className="chip">
                {q}
              </Link>
            </li>
          ))}
      ...
      <section className="grid grid-cols-1 gap-6 sm:grid-cols-3 sm:gap-8" aria-label="Arşiv durumu">
      ...
      <section id="bolumler" className="space-y-5">
        ...
        <ul className="grid gap-x-8 sm:grid-cols-2 lg:grid-cols-3">
          {recent.map((e) => (
            <li key={e.key} className="border-b border-line">
              <Link
                href={episodeHref(e.series, e.no)}
                className="flex min-h-11 items-center justify-between gap-4 py-2 font-body text-lg text-fg hover:text-moon"
              >
                <span>
                  {seriesShort(e.series)} · {e.no}. Bölüm
                </span>
                <span className="font-mono text-xs text-fg-3 tabular">{Math.round(e.duration_sec / 60)} dk</span>
              </Link>
            </li>
          ))}
        </ul>
...
function Stat({ label, value, note }: { label: string; value: string; note: string }) {
  return (
    <div className="space-y-1">
      ...
      <div className="font-mono text-xs text-fg-2">{note}</div>
    </div>
  );
}
